...
{
    private (int idxdate, Matrix<double>Cov)[] ComputeCovariances(Frame<DateTime, string> portfolioSeries,
        int nCovarianceWindow)
    {
    ...
    public IEnumerable<PortfolioOptimizationResult> Run(Frame<DateTime, string> portfolioSeries, int nCovarianceWindow,
        double targetVol,
        IDictionary<DateTime, Series<string, double>>? weightModulationSignal = null)
    {
        var rng = new ContinuousUniform(0, 1);
        var n = portfolioSeries.ColumnCount;
        var w0 = new double[n];
        rng.Samples(w0);
